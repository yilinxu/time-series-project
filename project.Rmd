...
```{r}
train_gdp<-window(reorg_ts[,'log_gdp'],end=c(2009,4))
test_gdp<-window(reorg_ts[,'log_gdp'],end=c(2010,1))

train_consop<-window(reorg_ts[,'log_consop'],end=c(2009,4))
test_consop<-window(reorg_ts[,'log_consop'],end=c(2010,1))

train_recprob<-window(reorg_ts[,'log_recprob'],end=c(2009,4))
test_recprob<-window(reorg_ts[,'log_recprob'],end=c(2010,1))

var_model<-VAR(cbind(train_gdp,train_consop,train_recprob),p=2,type="both")
summary(var_model)
```
...
#### Predict 
```{r}
forecast_obj<-forecast(var_model,h=49)
log_gdp_obj = forecast_obj[[2]]$train_gdp
log_gdp_fc = log_gdp_obj[3]$mean
autoplot(train_gdp) + autolayer(log_gdp_fc ,series="VAR forecast")
log_rec_obj = forecast_obj[[2]]$train_recprob
log_rec_fc = log_rec_obj[3]$mean
autoplot(train_recprob) + autolayer(log_rec_fc ,series="VAR forecast")
accuracy(test_gdp,log_gdp_fc)
accuracy(test_recprob,log_rec_fc)
```
